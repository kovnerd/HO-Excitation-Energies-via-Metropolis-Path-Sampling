...
void metropolis(double *x, double epsilon, gsl_rng *r)
{
	for(int i = 0; i < N; i++)
	{
		double oldX = x[i];
		double oldS = S(x, i);
		x[i] += epsilon * (2 * gsl_rng_uniform(r) - 1.);// generates random number between -epsilon and epsilon
...
void bootstrap_G(double** Gs, double** bootstrap, int n_configs, gsl_rng *r)
{
	...
	{
		alpha = (int) (gsl_rng_uniform(r)*(n_configs));//choose random config from Gs
		for(int n = 0; n < N; n++)//append random config to bootstrap
			bootstrap[i][n] = Gs[alpha][n];
	}
}
...
{	
	...
	double **energy_bootstrap = matrix_allocate_double(N - 1, N_error);
	...
	for(int n = 0; n < N - 1; n++)
		energy_bootstrap[n][n_err] = log(fabs(avG[n]/avG[n + 1]));
	}
	...
	for(int n_err = 0; n_err < N_error; n_err++)//energy error analysis
	{
		...
		Gs_average(Gs_bins, G_temp, N_configs/bin_size);//get propagator values from binned bootstrap copy of Gs
		
		for(int n = 0; n < N; n++)
			energy_bootstrap[n][n_err] = (n < N - 1) ? log(fabs(G_temp[n]/G_temp[n + 1])) :  log(fabs(G_temp[n]/G_temp[0]));//generate sample of energies
	}*/
	...
	for(int n = 0; n < N - 1; n++)
	{
		...
		energy[n] = log(fabs(avG[n]/avG[n + 1]));
		energy_error[n] = gsl_stats_sd(temp, 1, (size_t) N_error);
		printf("%f %f %f %f\n", n * LATTICE_SPACING, avG[n], energy[n]/LATTICE_SPACING, energy_error[n]/(LATTICE_SPACING));
	}
